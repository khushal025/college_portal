...
def allowed_file(filename):
    ...
    return '.' in filename and filename.rsplit('.', 1)[22].lower() in ALLOWED_EXTENSIONS  # [2]
...
def ping_db():
    ...
    if not conn:
        return 'DB connection failed', 500  # [9]
    cur = conn.cursor()
...
@app.route('/')
def index():
    return render_template('index.html')  # [9]

@app.route('/student_login')
def student_login_page():
    return render_template('student_login.html')  # [9]

@app.route('/teacher_login')
def teacher_login_page():
    return render_template('teacher_login.html')  # [9]

@app.route('/student_dashboard')
def student_dashboard():
    if 'student_id' not in session or session.get('user_type') != 'student':
        return redirect('/student_login')  # [9]
...
def api_teacher_login():
    try:
        ...
        session['teacher_id'] = t.get('teacher_id') or t.get('id') or None
        return jsonify({'success': True})
    except Exception as e:
...
def update_student_profile():
    try:
        ...
        """
        cur.execute(q, (
            data.get('student_name'), data.get('father_name'),
            data.get('course'), data.get('semester'), data.get('email'),
            roll_no
        ))
        conn.commit()
        cur.close(); conn.close()
...
def load_table_from_excel(path):
    ...
    # For .xlsx reading, ensure 'openpyxl' installed if needed
    return pd.read_excel(path)  # [18]

@app.route('/upload_attendance', methods=['POST'])
def upload_attendance():
    if 'file' not in request.files:
        return jsonify({'success': False, 'message': 'No file selected'}), 400  # [2]
